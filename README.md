...
### 每日热题：在变化中看见自己的选择方式（下一阶段）

主页将在「沙盘报告」旁增加「每日热题」入口。系统把适合参与的知乎热榜内容抽象为变化、关系、边界、取舍等心理主题，再生成经过安全校验的沙盘挑战。
...
现有自由创作模式保持不变；「每日热题」将作为并列入口，为希望获得引导的玩家提供情境化玩法。两种模式共用沙具库、沙盘编辑器、AI 分析、作品发布和好友系统。
...
现有平台能力的实施记录见 [后续开发实施计划](docs/superpowers/plans/2026-09-13-hackathon-development-roadmap.md)，下一阶段玩法设计见 [「选择镜像」玩法规格](docs/superpowers/specs/2026-09-15-choice-mirror-game-design.md)。后续优先顺序：

1. 在首页增加与「沙盘报告」并列的「每日热题」入口。
2. 接入知乎热榜主题转译、安全过滤和每日挑战降级机制。
3. 记录挑战前后的沙盘变化，生成可由玩家确认或修正的选择观察。
4. 生成默认私密、可变化且可选择公开的选择原型。
5. 让挑战沙盘复用作品发布、社区互动和好友申请流程。
6. 将用户主动公开的选择原型作为可选好友匹配信号。
