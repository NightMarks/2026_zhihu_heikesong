...
一个把知乎内容探索、自由沙盘创作、AI 心理分析与兴趣社交连接成完整体验的互动叙事游戏。

玩家从随机沙具出发，通过阅读和回应知乎内容获得灵感值、解锁 8 类共 96 件沙具，在沙盘中自由布置，并生成温和、非诊断式的 AI 沙盘报告。完成的作品可以发布到线上社区，由不同设备上的登录用户共同浏览、匹配和互动。

> 本项目参加知乎黑客松「跨次元游乐场：游戏与互动叙事」赛道。

## 核心创新

### AI 不只看结果，也寻找有证据的心理线索

传统沙盘应用往往只解释最终画面。「沙游心语」同时记录沙具的摆放顺序、移动、移除、缩放、旋转和玩家自述，再由 AI 按“客观证据 → 多种可能 → 邀请确认”的方式生成六维心理探索报告。

AI 不会根据一件沙具给玩家贴人格标签，也不会进行心理诊断。玩家自己的解释始终优先，报告中的联想必须能回到真实画面或操作过程。

### 知乎内容真正进入游戏资源循环

知乎不是登录入口或外部链接装饰，而是获取沙具和形成创作主题的内容来源：

- 从知乎搜索、热榜和个人创作中发现人物、建筑、自然、交通、象征和幻想等主题；
- 阅读内容并记录触动自己的观点，获得对应分类的灵感值；
- 在知乎完成真实创作后，将本人内容链接带回游戏验证并获得额外灵感；
- 使用灵感值解锁沙具，再把阅读产生的联想带进自己的沙盘。

平台没有开放的点赞、评论和发布能力仍由玩家本人在知乎完成，项目不模拟站内写入操作。

### 从一座沙盘找到产生共鸣的人

作品发布后，系统会根据双方主动公开的沙具类别、具体元素、空间布局、密度和报告关注方向计算结构相似度。玩家可以查看匹配理由、发送好友申请，并在双方同意后分享“仅好友可见”的作品。

匹配分数只表示作品表达上的相似，不推断性格，也不判断两个人是否“合适”。社区同时支持点赞、收藏、评论、分享和跨设备浏览。

### 每日热题：在变化中看见自己的选择方式

主页已在「沙盘报告」旁增加「每日热题」入口。系统每天读取知乎热榜，过滤不适合游戏化的风险主题，把可参与内容抽象为变化、关系、边界、取舍等心理主题，并生成四轮沙盘挑战；热榜不可用或没有安全候选时自动使用内置命题，保证玩法可进入。

玩家不会点击心理测试答案，而是直接移动、增加、删除或重组沙具完成多轮情境。系统对照相似情境中的操作差异，用“事实 → 可能性 → 玩家确认”帮助玩家认识自己反复出现的选择模式。

```text
知乎热榜 → 安全主题转译 → 建立初始沙盘 → 多轮镜像情境
→ 对照选择过程 → 确认或修正 AI 观察 → 获得可变化的选择原型
→ 发布挑战沙盘 → 匹配作品与好友
```

挑战完成后的沙盘继续复用现有发布、评论、收藏、共鸣匹配和好友申请流程。选择原型默认私密；用户可以主动公开某些原型，也可以在匹配时选择是否把公开原型作为一项共鸣信号。其他玩家只能看到对方主动公开的标签。

选择原型描述近期反复出现的行为，例如“边界守望者”“主动连接者”或“核心守护者”。它们基于多局、跨情境且经过玩家本人确认的观察生成，会随新选择变化，不是固定人格或心理诊断。
...
现有自由创作模式保持不变；「每日热题」作为并列入口，为希望获得引导的玩家提供情境化玩法。两种模式共用沙具库、沙盘编辑器、报告、作品发布和好友系统。

### 每日热题与选择镜像

每日挑战由“建立画面—发生变化—再次选择—回看过程”四轮组成。系统记录每轮沙具的增加、移除、移动、缩放、旋转和摆放顺序，以事实证据生成温和的选择观察；玩家可以确认或拒绝每条观察，拒绝内容不会进入个人画像。

只有经过本人确认、并在多局与不同情境中重复出现的观察，才会汇总为可变化的“选择原型”。原型默认私密，玩家可以分别决定是否公开、是否用于匹配，也可以删除已有记录。每日热题完成后的沙盘可直接复用原有发布、社区互动、共鸣匹配和好友申请流程。
...
已解锁的沙具会进入沙具库。桌面端可把沙具直接拖入沙盘，触屏设备仍可点选后放置；已放入的沙具可以移动、旋转、缩放和调整图层，并支持 50 步撤销/重做与 PNG 图片导出。页面使用去除白底后的透明 WebP 沙具素材；原始图片保存在根目录 `images/`，可用 `npm run assets:prepare` 重新生成网页资源。

完成布置后会直接生成完整报告，无需手动选择分析方向。大模型按六个部分输出：客观观察与整体主题、空间布局、沙具选择/大小/关系、沙子水与制作过程、综合心理动力与资源、开放问题。每个心理联想都需要给出画面或过程证据，再提出可能性并邀请用户确认。沙盘页还可填写个人自述并记录摆放顺序和编辑动作；个人自述始终优先于通用象征。
...
- 8 类 96 件图片沙具、随机探索、灵感值和逐件解锁
- 知乎搜索、热榜和离线示例内容
- Windows/macOS 自动发现并调用本地 `zhihu-cli`
- 知乎 OAuth 登录，严格使用官方授权参数，并通过一次性同源确认和安全 Session Cookie 防止登录注入
...
│     ├─ challenge.js   每日热题四轮流程与恢复
│     ├─ tray-diff.js   沙盘轮次差异计算
│     ├─ choice-profile.js 选择原型确认与隐私设置
│     └─ community.js   线上社区、发布和旧作品迁移
├─ images/             原始沙具图片，仅用于开发和重新处理
├─ scripts/            素材去白底与构建辅助脚本
├─ server/
│  ├─ index.js         Express 服务、路由、OAuth 和验证
│  ├─ community.js     社区业务、隐私与分页
│  ├─ challenges.js    热榜筛选、每日命题与降级
│  ├─ challenge-runs.js 挑战进度、观察确认与画像接口
│  ├─ archetypes.js    选择原型聚合规则
│  ├─ store.js         原子写入的 JSON 持久化存储
│  ├─ zhihu.js         知乎开放平台 API 客户端
│  ├─ cli-source.js    跨平台本地 zhihu-cli 数据源
│  └─ oauth-state.js   OAuth 登录 nonce、时效与同源确认校验
├─ docs/               后续开发计划和服务器部署指南
├─ Dockerfile          容器化部署入口
...
如果跳转后出现知乎“网络环境异常风险”安全验证，说明请求仍停留在知乎登录侧，尚未进入本项目的回调。请关闭 VPN/代理、允许知乎 Cookie，先在同一浏览器正常登录知乎，再重新点击授权；项目代码不能也不会绕过知乎验证码。

如果知乎授权页直接显示“出错了！请稍后再试”，优先核对申请 App ID 时提交的回调地址是否与 `ZHIHU_REDIRECT_URI` **逐字符一致**（协议、主机、端口、路径和末尾斜杠都必须一致）。本站只发送知乎当前文档声明的 `redirect_uri`、`app_id`、`response_type` 三个参数；授权成功回站后还会显示一次账号确认页，确认后才建立登录会话。
...
| GET | `/api/challenges/daily` | 获取当天的知乎热榜沙盘挑战 |
| POST | `/api/challenge-runs` | 创建每日挑战记录 |
| PUT | `/api/challenge-runs/:runId/rounds/:roundId` | 保存一轮沙盘快照与差异 |
| POST | `/api/challenge-runs/:runId/reflection` | 生成基于操作证据的选择观察 |
| PUT | `/api/challenge-runs/:runId/confirmations` | 确认或拒绝选择观察 |
| GET | `/api/challenge-runs/profile` | 获取个人选择记录与原型 |
| PATCH | `/api/challenge-runs/profile/archetypes/:id` | 设置单个原型的公开与匹配权限 |
| DELETE | `/api/challenge-runs/profile/records/:id` | 删除个人选择记录 |
...
- 每日挑战命题、挑战记录、确认结果与原型隐私设置保存在服务器数据卷；未完成轮次的本地草稿也会在浏览器中恢复。
...
平台能力的实施记录见 [后续开发实施计划](docs/superpowers/plans/2026-09-13-hackathon-development-roadmap.md)；每日热题的产品边界、心理安全和隐私设计见 [「选择镜像」玩法规格](docs/superpowers/specs/2026-09-15-choice-mirror-game-design.md)，代码实施步骤见 [每日热题实施计划](docs/superpowers/plans/2026-09-15-daily-hot-choice-mirror.md)。

下一步可在保持“玩家拥有最终解释权”的前提下，继续优化热榜主题转译质量、补充更多挑战模板，并将单机 JSON 存储升级为适合多实例部署的数据库。
