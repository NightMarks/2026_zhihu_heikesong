...
如果跳转后出现知乎“网络环境异常风险”安全验证，说明请求仍停留在知乎登录侧，尚未进入本项目的回调。请关闭 VPN/代理、允许知乎 Cookie，先在同一浏览器正常登录知乎，再重新点击授权；项目代码不能也不会绕过知乎验证码。若回到本站后显示 `state` 或 token 错误，再核对开放平台登记的回调地址必须与 `ZHIHU_REDIRECT_URI` 逐字符一致。
