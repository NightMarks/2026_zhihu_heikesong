...
申请 OAuth App ID 时，必须把下面的地址作为授权回调地址提交给知乎：

```text
https://212.129.255.239/auth/callback
```

OAuth 回调不是只在服务器 `.env` 中配置即可：知乎侧登记值和服务器的 `ZHIHU_REDIRECT_URI` 必须逐字符相同。如果 App ID `504` 申请时登记的不是这个地址，请联系 `openplatform@zhihu.com` 更新；平台拒绝 IP 回调时，请使用文末的“免费固定隧道地址”方案并登记该固定 HTTPS 地址。
